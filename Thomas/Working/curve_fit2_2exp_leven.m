function [decay_ms, decay_ms2 standd] = curve_fit2_2exp_leven(xdata,ydata)
...
expfun = @(p) p(1)*exp(-xdata/p(2)) + p(3)*exp(-xdata/p(4)) + p(5) - ydata; % Objective Funciton
...
[p,resnorm,residual,exitflag,output]  = lsqnonlin(expfun,pguess,[],[],options);

yfitted = expfun(p) + ydata;
